    ...
    <Table>
      <thead>
        <tr className="text-left border-b bg-slate-50">
          ...
          <th className="p-2">Temple Missing Actions</th>
        </tr>
      </thead>
      <tbody>
        {sorted.map((r) => <tr key={r.id} className="border-b"><td className="p-2 font-medium">{r.username}</td><td className="p-2">{r.status}</td><td className="p-2">{r.currentRank}</td><td className="p-2">{r.eligibleRank}</td><td className="p-2">{fmt(r.lastSeen)}</td><td className="p-2">{r.status === "MISSING_PENDING_REVIEW" ? <div className="flex gap-2"><button disabled={busy} className="px-3 py-1.5 rounded-md bg-emerald-600 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/temple-missing/add`, { method: "POST" }))}>Add Back</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-rose-700 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/temple-missing/remove-db`, { method: "POST" }))}>Remove from DB</button></div> : "-"}</td></tr>)}
      </tbody>
    </Table>
  );
}
