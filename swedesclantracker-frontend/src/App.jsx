  ...
  return (
    <Table>
      ...
      <tbody>
        {sorted.map((r) => {
          const mergeMeta = (() => {
            try { return r.mergeMetadataJson ? JSON.parse(r.mergeMetadataJson) : null; } catch { return null; }
          })();
          ...
          return <tr key={r.id} className="border-b"><td className="p-2 font-medium">{r.username}</td><td className="p-2">{r.status}</td><td className="p-2">{r.currentRank}</td><td className="p-2">{r.eligibleRank}</td><td className="p-2">{fmt(r.lastSeen)}</td><td className="p-2">{r.discordCardState ?? "-"}</td><td className="p-2">{(r.status === "MISSING_PENDING_REVIEW" || r.status === "NEW_PENDING_REVIEW") ? <div className="flex gap-2"><button disabled={busy} className="px-3 py-1.5 rounded-md bg-emerald-600 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/temple-missing/add`, { method: "POST" }))}>Add to Temple</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-rose-700 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/temple-missing/remove-db`, { method: "POST" }))}>Remove from DB</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-slate-700 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/requeue-discord-card`, { method: "POST" }))}>Requeue Card</button></div> : r.status === "MERGE_SUGGESTED" ? <div className="flex gap-2 flex-wrap"><button disabled={busy} className="px-3 py-1.5 rounded-md bg-emerald-600 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/merge/confirm`, { method: "POST" }))}>Confirm rename</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-blue-700 text-white disabled:bg-slate-400" onClick={() => { const options = candidatePreviousPlayers.map((x) => x.PreviousPlayer).filter(Boolean); const pick = window.prompt(`Choose previous player:\\n${options.join("\\n")}`, suggestedPrevious || ""); if (!pick) return; action(() => call(`/review/players/${r.id}/merge/reassign`, { method: "POST", body: JSON.stringify({ previousUsername: pick }) })); }}>Pick other</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-slate-700 text-white disabled:bg-slate-400" onClick={() => { const manual = window.prompt("Manual previous username:", ""); if (!manual) return; action(() => call(`/review/players/${r.id}/merge/manual`, { method: "POST", body: JSON.stringify({ previousUsername: manual }) })); }}>Manual previous</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-rose-700 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/merge/abort`, { method: "POST" }))}>Abort rename</button><button disabled={busy} className="px-3 py-1.5 rounded-md bg-slate-700 text-white disabled:bg-slate-400" onClick={() => action(() => call(`/review/players/${r.id}/requeue-discord-card`, { method: "POST" }))}>Requeue Card</button></div> : "-"}</td></tr>;
        })}
      </tbody>
    </Table>
  );
